...
def solver(mesh,V,n0,c_k,dt,T,save_interval,times,doses,nfile,cfile):
    ...
    while(t<n_steps):
        ...
        a_n = 0.5*n*v*dx + 0.5*dt*Dxn*inner(grad(n)[0],grad(v)[0])*dx + 0.5*dt*Dsn*inner(grad(n)[1],grad(v)[1])*dx \
             + 0.5*dt*grad(vs*n)[1]*v*dx - 0.5*dt*n*v*F*dx - n*v*S_rt*dx  + dt*n*v*vs*ds(0) - dt*n*v*vs*ds(2) 
        L_n = n0*v*dx - 0.5*n0*v*dx  - 0.5*dt*Dxn*inner(grad(n0)[0],grad(v)[0])*dx - 0.5*dt*Dsn*inner(grad(n0)[1],grad(v)[1])*dx \
        -  0.5*dt*grad(vs*n0)[1]*v*dx + 0.5*dt*n0*v*F*dx 
