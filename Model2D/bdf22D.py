...
class Solver2D:
    ...
    def solve(self):
        ...
        L_c = Vc*w*dx #- self.gamma*0.5*phi_h*w*dx#- self.gamma*(0.5 + 0.5*tanh((self.c_k-self.c_N)/0.05))*phi_h*w*dx
        eps= 1.0
        tol = 1.0E-3
        iter = 0        
        maxiter = 30    
        ...
        a_n = n*v*dx + self.dt*self.Dxn*inner(grad(n)[0],grad(v)[0])*dx + self.dt*self.Dsn*inner(grad(n)[1],grad(v)[1])*dx \
                + self.dt*self.Dsn*inner(grad(n)[2],grad(v)[2])*dx \
                + self.dt*grad(vs*n)[1]*v*dx - self.dt*n*v*F*dx - n*v*S_rt*dx  + self.dt*n*v*vs*self.ds(5) - self.dt*n*v*vs*self.ds(4) 
        L_n = self.n0*v*dx
        solve(a_n==L_n,N)
        n1.assign(N)
        t += 1
