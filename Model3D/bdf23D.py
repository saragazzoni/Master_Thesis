...
class Solver3D:
    ...
    def solve(self):
        ...
        n1_array = [Function(self.V) for _ in range(Ns+1)]
        for s in range(Ns+1):
            self.n0.s = s*dz
        ...
        sum = 0
        for s in range(Ns+1):
            n1_array[s].vector()[:] = x[nmesh*s:nmesh*(s+1)]
        ...
        while(t < n_steps):
            ...
            for s in range(2):
                ...
                a00 = (3/(2*self.dt))*n*v*dx + self.Dxn*inner(grad(n),grad(v))*dx + self.Dsn/(2*dz*dz)*n*v*dx - n*v*F*dx 
                a01 = -self.Dsn/(2*dz*dz)*n*v*dx + 1/(2*dz)*vs*n*v*dx
                A0 = assemble(a00)
                A0r = assemble(a01)
                row1 = [0] * (Ns+1)
            ...
            for s in range(2,Ns-1):
                ...
                vs = Expression('V_plus*tanh(s/csi_plus)*tanh((1-s)/csi_plus)*(0.5+0.5*tanh((c-c_H)*pow(epsilon,-1))) \
                        ...
                        epsilon=self.epsilon,c=C,s=(s+1)*dz,degree=2)
                F = Expression("P - K", degree=2, P=P, K=K)
                a = (3/(2*self.dt))*n*v*dx + self.Dxn*inner(grad(n),grad(v))*dx + 2*self.Dsn/(2*dz*dz)*n*v*dx - n*v*F*dx
                ar = -self.Dsn/(2*dz*dz)*n*v*dx + 1/(2*dz)*vs*n*v*dx
                vs = Expression('V_plus*tanh(s/csi_plus)*tanh((1-s)/csi_plus)*(0.5+0.5*tanh((c-c_H)*pow(epsilon,-1))) \
                        - V_minus*tanh(s/csi_minus)*tanh(pow(1-s,2)/csi_minus)*(0.5+0.5*tanh((c_H-c)*pow(epsilon,-1)))',
                        V_plus=self.V_plus,V_minus=self.V_minus,csi_minus=self.csi_minus,csi_plus=self.csi_plus,c_H=self.c_H,
            ...
            while s <= Ns:
                ...
                aNN = (3/(2*self.dt))*n*v*dx + self.Dxn*inner(grad(n),grad(v))*dx + self.Dsn/(2*dz*dz)*n*v*dx - n*v*F*dx
                vs = Expression('V_plus*tanh(s/csi_plus)*tanh((1-s)/csi_plus)*(0.5+0.5*tanh((c-c_H)*pow(epsilon,-1))) \
                            - V_minus*tanh(s/csi_minus)*tanh(pow(1-s,2)/csi_minus)*(0.5+0.5*tanh((c_H-c)*pow(epsilon,-1)))',
                            V_plus=self.V_plus,V_minus=self.V_minus,csi_minus=self.csi_minus,csi_plus=self.csi_plus,c_H=self.c_H,
                            epsilon=self.epsilon,c=C,s=(s-1)*dz,degree=2)
            ...
            b_list = []
            for s in range(Ns+1):
                L = (4/(2*self.dt))*n1_array[s]*v*dx  - (1/(2*self.dt))*n0_array[s]*v*dx
                b_curr = assemble(L)
                b_list.append(b_curr)
            b = block_vec(b_list)
